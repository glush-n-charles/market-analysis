...
def process_chart(file_path):
    ...
    if (len(df) < 1000):
        ...
        if (PRINT_HELPER_TEXT_TWO):
            print('NOT ENOUGH DATA POINTS IN THIS BITCH.\n')
        return
    ...
    # decrease amount of points we are working on.
    ...
    # further decrease num of points we are working on.
    X, Y = zipp(yl, yh, xl, xh)

    if (PRINT_HELPER_TEXT):
        print('ZIPPED THAT SHIT UP')

    # break chart up into patterns.
    decy, decx, decsex, decsey = findDec(vl, X, Y, 0.05, PATTERN_SIZE)

    if (PRINT_HELPER_TEXT):
        print('BROKE THAT BIH UP INTO PATTERNS')
    
    # clean up resulting patterns so they are all 1000 in length.
    connect_patterns(decx, decy, PATTERN_SIZE)

    if (PRINT_HELPER_TEXT):
        print('CONNECTED THE MF PATTERNS')
...
for filename in os.scandir(directory):
    if filename.is_file():
        ...
        # cancel timer if function succeeded in under 5 seconds.
        signal.alarm(0)
